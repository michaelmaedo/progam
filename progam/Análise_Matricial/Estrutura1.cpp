...
//Declaração de registros para as barras equivalente ao common utilizado em fortran
extern struct elemento{
	int noinicio, nofim;
...
int main()
{
	...
	cout << '\n'
		 ...
		 << "                  Herman P. Pissinin   RA: 123456\n"
		 << "                  Ricardo V. Sartori   RA: 123456" << endl;

	/*Declaração de variáveis: Dados relativos à estruturas => n⁰ de nós, n⁰ de barras, n⁰ de nos com carga, 
        n⁰ de barras com carga, n⁰ de nos com vinculação, conectividade das barras, coordenadas dos nos e propriedades das barras*/
	int Nnos, TresNnos, Nbarras, Nnocar, Nbarcar, Nnovinc, *noinicio, *nofim, *vinc;
	float *x, *y, **Rig, *ANE, *px1, *px2, *py1, *py2, *Rig2;
	...
	ler_dados1( entrada, saida, &Nnos, &Nbarras, &Nnocar, &Nbarcar, &Nnovinc );
	...
	ler_dados2( entrada, saida, x, y, elemento, ANE, px1, px2, py1, py2, vinc, Nnos, Nbarras, Nnocar, Nbarcar, Nnovinc );

	//Função para calcular a matriz de rigidez não restringida da estrutura
	Rig = calcula_Rig( Nnos, Nbarras, elemento, Rig );

	//Função para o vetor de forças nodais
	ANE = monta_ANE( Nbarras, px1, px2, py1, py2, elemento, ANE);

	//Função para vincular os nós necessários
	vinculacao( vinc, ANE, Rig, Nnos);
	...
	saida_dados( saida, ANE, Rig, Nnos );
